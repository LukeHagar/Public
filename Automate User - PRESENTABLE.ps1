...
function New-Employee
{
	...
	try{
		...
		$LocationObject = $null
		$ADGroups = @()
		$confirm = ("yes", "y", "indeed", "forsure", "go", "go ahead", "please do")
	}
	catch{ Write-Host "[ERROR] Error in variable Block - $($_.Exception.Message)" }
	...
	catch{	Write-Host "[ERROR] The script was unable to connect to one of various vital services. - $($_.Exception.Message)" }
	...
	Get-ADPrincipalGroupMembership $groupCopy | ForEach-Object { if ($_.Name -ne "Domain Users") { $ADGroups += $_.Name } }	
	
	# Create a new user in AD
	$shouldAD = Read-Host -Prompt "Would you like to create an Active Directory account for this user? [y/n]"
	if ($confirm -contains $shouldAD)
	{
		try
		{
	...
	$shouldFile = Read-Host -Prompt "Would you like a folder to be generated for the user? [y/n]"
	if ($confirm -contains $shouldFile)
	{
	...
	$shouldMS = Read-Host -Prompt "Do you want an O365 account to be made for the user? [y/n]"
	if ($confirm -contains $shouldMS)
	{
		Write-Host "[INFO] Generating mailbox.." -ForegroundColor Black -BackgroundColor Red
		New-MsolUser $MSOlObject
		do
		{
			$check = "valid"
	...
	$shouldMail = Read-Host -Prompt "Finally, would you like the Admin to be notifed of this creation, and the password disclosed? [y/n]"
	if ($confirm -contains $shouldMail)
	{
		...
		Send-MailMessage $emailObject -UseSsl
	}
